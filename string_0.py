...
def donuts(count):
    ...
    # if count < 10:
        # res = u'Количество пончиков: {}'.format(count)
    # else:
        # res = u'Количество пончиков: много'
    # return res
    
    # --- Вариант 2 -- в одну строку через тернарный оператор:
    return u'Количество пончиков: {}'.format(count if count < 10 else u'много')
...
def both_ends(s):
    ...
    # return res
    # Вариант 2:
    return ''.join((s[0:2], s[-2:])) if len(s) > 1 else ''
...
def fix_start(s):
    # +++ ваш код +++
    # Возможны разные варианты, но не стоит усложнять
    res = s[1:]
    res = res.replace(s[0],'*')
    return ''.join((s[0],res))
...
def mix_up(a, b):    
    # +++ ваш код +++
    # Допустимы также некоторые варианты
...
def not_bad(s):
    ...
    ne = s.find(u'не')
    ploh = s.find(u'плох') 
    if -1 < ne < ploh:          # учитываем, что 'не' и 'плох' могут не встретиться в строке
        s = ''.join((s[0:ne], u'хорош', s[ploh+4:]))
    return s
...
def front_back(a, b):
    # +++ ваш код +++
    # введем доп. переменные для читаемости
    mid_a = len(a) // 2 + len(a) % 2        # len(a) % 2   возвращает 1 или 0  - можно это учесть и обойтись без if
    mid_b = len(b) // 2 + len(b) % 2
    return ''.join((a[:mid_a], b[:mid_b], a[mid_a:], b[mid_b:]))
...
def main():
    ...
    test(donuts(9), u'Количество пончиков: 9')
    test(donuts(10), u'Количество пончиков: много')
    test(donuts(99), u'Количество пончиков: много')
    ...
    print()
    print('Хорош')
    test(not_bad(u'Этот фильм не так уж плох'), u'Этот фильм хорош')
    test(not_bad(u'А ужин был не плох!'), u'А ужин был хорош!')
    test(not_bad(u'Этот чай уже не горячий'), u'Этот чай уже не горячий')
    test(not_bad(u"Этот плох, но не совсем"), u"Этот плох, но не совсем")
